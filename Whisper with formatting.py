...
def format_transcription(text):
    # Define patterns for salutations and sign-offs
    salutations = ['Hi', 'Hallo', 'Hello', 'Servus']
    sign_offs = ['Kind regards', 'Freundliche Grüße', 'Best regards', 'Regards']

    # Create regular expressions to match salutations and sign-offs
    salutation_pattern = r'(?i)\b(' + '|'.join(salutations) + r')\s+[A-Za-z]+'
    sign_off_pattern = r'(?i)(' + '|'.join(sign_offs) + r')'

    # Insert two line breaks after salutations
    text = re.sub(salutation_pattern, r'\g<0>,\n\n', text)

    # Insert two line breaks before sign-offs
...
def transcribe():
    ...
    pyperclip.copy(formatted_text)  # Copy the formatted transcription text to the clipboard
...
# Bind the Ctrl + t hotkey to the toggle recording function within the Tkinter window
root.bind('<Control-q>', lambda event: toggle_recording())

# Set up a global hotkey using the keyboard library
